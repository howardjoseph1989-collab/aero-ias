...
|----------|-----------------------------|
| **Gemini (default)** | Recommended brain/voice path. Server-brokered `GEMINI_API_KEY` / `GOOGLE_API_KEY` (not the Maps key). Mic audio is posted to `/api/gemini/turn`, **Gemini 3.6 Flash** (`gemini-3.6-flash`, override with `GEMINI_VOICE_MODEL`) `generateContent` may call the **same 28 gevActions tools**, then a spoken reply (dedicated Gemini TTS — default `gemini-2.5-flash-preview-tts` via `GEMINI_TTS_MODEL` — when available, otherwise the browser SpeechSynthesis API). Gemini 3.6 Flash accepts audio input but does not generate audio, so TTS stays on a TTS-capable model. Turns are send-on-pause / hold-Space. Gemini Live WebSocket exists upstream but does not mint a browser-safe WebRTC ephemeral token the way OpenAI does, so AERO IAS uses this smallest working tool-calling path instead. |
...
| 🟡 | **Gemini** | 🎙️ **Recommended voice+tools.** Defaults to **Gemini 3.6 Flash** (`gemini-3.6-flash`; override with `GEMINI_VOICE_MODEL`). Mic audio → same 28 tools → spoken reply via dedicated TTS (`GEMINI_TTS_MODEL`, default `gemini-2.5-flash-preview-tts`). Set `GEMINI_API_KEY` (or `GOOGLE_API_KEY` as an alias). **Not** the Maps key | [Google AI Studio](https://aistudio.google.com/apikey) — free tier with quotas |
...
| **🟡 Gemini voice** | **The free-tier path.** Google AI Studio keys have a free quota. AERO IAS uses turn-based `generateContent` on **Gemini 3.6 Flash** (audio in, tools, then dedicated TTS or browser speech). It is not billed like OpenAI Realtime; the mic chrome shows `TURN` instead of a session dollar meter. Provider quotas still apply — set `GEV_RATELIMIT_GEMINI_PER_MIN` if you share the server. |
...
AERO IAS runs on **public data, clear sources, and local-first execution.** No secrets, no private datasets, no mystery scraping — anything involving a private key is brokered server-side. It has the visual grammar of a classified ops room, built entirely from open signals and inspectable code.
