# AERO IAS — Testing Guide (voice map-annotation whiteboard + live tracking)
...
- **Voice (the real feature):** click **AERO MIC** (bottom of screen) → wait for **LISTENING** →
...
1. While tracking a plane over Austin, AERO MIC: *"annotate downtown Austin."*
...
> All from the Austin default view (no camera move needed). Click AERO MIC, then speak.
...
- **No AERO MIC button / voice errors:** `OPENAI_API_KEY` didn't load — use the console API for
