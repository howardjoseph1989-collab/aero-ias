/**
 * Shared GEV / AERO IAS voice tool schemas.
 *
 * OpenAI Realtime and the Gemini turn broker must declare the same 28 tools so
 * action routing stays one runner (`gevActions.js`). Keep this file as the
 * single schema source; vite.config.js re-exports it into the Realtime session.
 */
export const GEV_REALTIME_TOOLS = [
  {
    type: 'function',
    name: 'fly_to_location',
    description: "Fly the AERO IAS camera to a known city, geocoded country/region/city/landmark, or explicit WGS84 coordinate. Countries/cities frame the whole place; landmarks/buildings use close framing.",
    parameters: {
      type: 'object',
  ...
  },
  {
    ...
    description: "Enable or disable one registered AERO IAS data layer.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: "Set the active AERO IAS visual filter/style.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
/** Number of voice tools the runner and both providers must stay aligned on. */
export const GEV_VOICE_TOOL_COUNT = GEV_REALTIME_TOOLS.length;

/**
 * Convert OpenAI Realtime function tools into Gemini functionDeclarations.
 * Drops `type: 'function'` and JSON Schema fields Gemini rejects.
 * @param {typeof GEV_REALTIME_TOOLS} [tools]
 * @returns {Array<{name: string, description: string, parameters: object}>}
 */
export function toGeminiFunctionDeclarations(tools = GEV_REALTIME_TOOLS) {
  return tools.map((tool) => ({
    name: tool.name,
    description: String(tool.description || ''),
    parameters: sanitizeGeminiSchema(tool.parameters || { type: 'object', properties: {} }),
  }));
}
...
function sanitizeGeminiSchema(schema) {
  ...
  if (out.items && typeof out.items === 'object') {
    out.items = sanitizeGeminiSchema(out.items);
  }
  return out;
}

/** Tool names only — used by tests to prove routing tables did not drift. */
export function gevVoiceToolNames(tools = GEV_REALTIME_TOOLS) {
