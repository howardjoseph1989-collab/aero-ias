/**
 * Vite configuration for AERO IAS — a cinematic geospatial app.
 *
...
import { GEV_REALTIME_TOOLS } from './src/voice/gevToolSchemas.mjs';
import {
  GEMINI_TURN_BODY_LIMIT,
  geminiVoiceStatus,
  resolveGeminiApiKey,
  runGeminiTurn,
...
/** Gemini voice turns. Null = unlimited (default). */
      ...
      try {
        ...
        const response = await fetch('https://api.openai.com/v1/responses', {
          ...
          body: JSON.stringify({
            ...
            instructions: [
              "Write one concise intelligence-HUD summary for AERO IAS.",
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      const sessionConfig = {
        session: {
          ...
          instructions: [
            "You are AERO Voice Control, a concise voice controller for a Cesium geospatial app called AERO IAS.",
...
/**
 * Gemini voice broker. The key stays server-side; the browser posts a recorded
 * turn (or tool results) to `/api/gemini/turn` and receives function calls
 * and/or spoken text. Full Gemini Live WebSocket/WebRTC parity is not used:
 * there is no OpenAI-style ephemeral WebRTC token for a browser client here.
 */
export function geminiVoiceProxy() {
  ...
  function install(middlewares) {
    middlewares.use('/api/voice/providers', (req, res) => {
      if (req.method !== 'GET') {
        writeJson(res, 405, { error: 'Method not allowed' });
        return;
      }
      const gemini = geminiVoiceStatus(process.env);
      writeJson(res, 200, {
    ...
    middlewares.use('/api/gemini/status', (req, res) => {
      if (req.method !== 'GET') {
        writeJson(res, 405, { error: 'Method not allowed' });
        return;
      }
      writeJson(res, 200, geminiVoiceStatus(process.env));
    });

    middlewares.use('/api/gemini/turn', async (req, res) => {
      if (req.method !== 'POST') {
        writeJson(res, 405, { error: 'Method not allowed' });
        return;
      }
      if (!resolveGeminiApiKey(process.env)) {
        writeJson(res, 503, { error: 'GEMINI_API_KEY (or GOOGLE_API_KEY) is not set' });
        return;
      }
      if (!enforceOptInRateLimit(geminiRateLimiter(), req, res)) return;
      try {
        const raw = await readRequestBody(req, GEMINI_TURN_BODY_LIMIT);
        const body = JSON.parse(raw || '{}');
        const result = await runGeminiTurn({ env: process.env, body });
        writeJson(res, result.status, result.payload);
      } catch (error) {
        writeJson(res, 400, { error: error?.message || 'Invalid Gemini turn' });
      }
    });
  }
...
// GEV_REALTIME_TOOLS is imported from src/voice/gevToolSchemas.mjs so the
// Gemini turn broker and OpenAI Realtime session declare the same 28 tools.
