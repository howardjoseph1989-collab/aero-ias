...
- Map-view chrome is one **white glass bottom toolbar**: Search, My Location,
  data layers, quick views, nav, mic, and every other clickable control live
  in a single wrapping strip with consistent gaps. Each group is floatable
  and draggable, clamped inside the viewport, and stays screen-fixed while
  the globe zooms or pans. First load starts everything docked at the bottom
  (not scattered). Exactly three quick views live on the bar: **GLOBAL**,
  **BIRDS EYE**, and **STREET**. **AERO MIC** stays the largest dock control
  (white idle, cyan/green listening, red error; label text stays white).
  Layers, Gemini + OpenAI, and the 28 voice tools are unchanged. Cockpit
  still uses the original left-lane instruments.
